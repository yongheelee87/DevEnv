...
def make_home_HTML(data: dict, export_path: str):
    ...
    lst_table = [
        '<table style="border: none;border-collapse: collapse;font-size: 1.0em;color: black;margin-bottom: 40px;margin-left: 20px;padding: 20px;">',
        '<tr style="background-color: #ECF0F1;border: 1px solid #54585d;">',
        '<th style="padding: 15px;border: 1px solid #54585d;text-align: center;">모듈</th>',
        '<th style="padding: 15px;border: 1px solid #54585d;text-align: center;">기능</th>',
        '<th style="padding: 15px;border: 1px solid #54585d;text-align: center;">결과</th>',
        '<th style="padding: 15px;border: 1px solid #54585d;text-align: center;">상세 링크</th>',
        '</tr>']
    ...
    <html lang="en">
        ...
        <body>
            <h1 style="font-family: 'Black Han Sans', sans-serif;font-size: 2.5em;color: red;margin: 20px 0 10px 10px;>{title}</h1>
            <h2 style="font-family: 'Black Han Sans', sans-serif;font-size: 1.0em;color: red;margin: 20px 0 10px 10px;>테스트 수행 날짜: {date}</h2>
    {sum_body}
            <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.2.2/dist/js/bootstrap.bundle.min.js" integrity="sha384-OERcA2EqjJCMA+/3y+gxIOqMEjwtxJY7qPCqsdltbNJuaOe923+mo//f6V8Qbsw3" crossorigin="anonymous"></script>
        </body>
    </html>
...
def _write_summary(df_sum):
    ...
    for line in lst_html:
        if '<td' in line:
            ...
            line = """<table class="dataframe" style="border: none;border-collapse: collapse;font-size: 1.0em;color: black;margin-bottom: 40px;margin-left: 20px;padding: 20px;">"""
...
def _write_img_body(dict_tc: dict):
    ...
    for lmg_src in dict_tc.keys():
        img_body = """
            <h3 style="font-family: 'Black Han Sans', sans-serif;font-size: 1em;color: red;margin: 0 0 0 10px;>{sub_title}</h3>
            <img src="{img_src}" width="1100" height="1100" style="width: 1100px; height: 880px; object-fit:cover;margin: 0 0 20px 0px;" alt="NOT FOUND"></img>
        """
